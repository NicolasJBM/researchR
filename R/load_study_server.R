...
load_study_server <- function(id, study_paths){
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$loadstudy, {
      ...
      if (base::length(study_paths()) != 2){
        ...
        translations <- tibble::tibble(
          file = base::list.files(study_paths()$subfolders$translations, full.names = FALSE)
        ) |>
        ...
        documents <- tibble::tibble(
          path = base::list.files(study_paths()$subfolders$originals, full.names = TRUE),
          file = base::list.files(study_paths()$subfolders$originals, full.names = FALSE)
        ) |>
